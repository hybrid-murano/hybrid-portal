...
class res_users(osv.Model):
    ...
    def _message_post_get_eid(self, cr, uid, thread_id, context=None):
        ...
        if context and 'thread_model' in context:
            ...
            context['thread_model'] = 'hybrid.user'
        if isinstance(thread_id, (list, tuple)):
            thread_id = thread_id[0]
        return self.pool.get('hybrid.user').search(cr, uid, [('user_id', '=', thread_id)], context=context)
    ...
    def message_post(self, cr, uid, thread_id, context=None, **kwargs):
        ...
        for employee_id in employee_ids:
            res = self.pool.get('hybrid.user').message_post(cr, uid, employee_id, context=context, **kwargs)
        return res
