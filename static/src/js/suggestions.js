openerp.hybrid = function(session) {
    ...
    suggestions.Employees = session.web.Widget.extend({
        ...
        init: function () {
            ...
            this.hybrid_user = new session.web.DataSetSearch(this, 'hybrid.user');
        ...
        fetch_suggested_employee: function () {
            var self = this;
            var employee = self.hybrid_user.call('get_suggested_thread', {'removed_suggested_threads': removed_suggested_employee}).then(function (res) {
                _(res).each(function (result) {
                    result['image']=self.session.url('/web/binary/image', {model: 'hybrid.user', field: 'image_small', id: result.id});
                });
                self.employees = res;
            });
        ...
        display_suggested_employees: function () {
            ...
            return this.$el.empty().html(QWeb.render('hybrid.suggestions.employees', {'widget': this}));
        },

        follow_employee: function (event) {
            var self = this;
            var employee_id = parseInt($(event.currentTarget).attr('id'), 10);
            return this.hybrid_user.call('message_subscribe_users', [[employee_id], [this.session.uid], undefined]).then(function(res) {
